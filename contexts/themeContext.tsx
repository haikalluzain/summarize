...
export const ThemeContextProvider: FC<IThemeContextProviderProps> = ({
  ...
  // @ts-ignore
  const mobileDesign =
    (deviceScreen?.width as any) <= process.env.MOBILE_BREAKPOINT_SIZE
  ...
  const [asideStatus, setAsideStatus] = useState(
    ...
    //   :
    // @ts-ignore
    deviceScreen?.width >= process.env.ASIDE_MINIMIZE_BREAKPOINT_SIZE
  )
  // useLayoutEffect(() => {
  ...
  //   // @ts-ignore
  //   if (
  //     (deviceScreen?.width as any) >= process.env.ASIDE_MINIMIZE_BREAKPOINT_SIZE
  //   ) {
