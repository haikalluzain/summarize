...
const useAsideTouch = () => {
  ...
  const mobileDesign =
    // @ts-ignore
    deviceScreen?.width <= Number(process.env.MOBILE_BREAKPOINT_SIZE)
  const hasTouchButton =
    // @ts-ignore
    deviceScreen?.width > Number(process.env.ASIDE_MINIMIZE_BREAKPOINT_SIZE)

  const asideWidthWithSpace =
    (parseInt(String(process.env.ASIDE_WIDTH_PX), 10) +
      parseInt(String(process.env.SPACER_PX), 10)) *
    -1

  const x = useMotionValue(
    process.env.ASIDE_TOUCH_STATUS === 'true' ? 0 : asideWidthWithSpace
  )
